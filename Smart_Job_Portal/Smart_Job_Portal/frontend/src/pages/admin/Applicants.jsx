...
const Applicants = () => {
    ...
    const fetchApplicants = async () => {
        try {
            ...
            // Enhance with AI Match results
            const enhanced = await Promise.all(data.map(async (app) => {
                try {
                    const { data: matchData } = await api.get(`/ai/match-result/${app.id}`);
                    return { ...app, aiScore: matchData ? matchData.match_percentage : null };
                } catch (e) {
                                                    ...
                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            {applicant.aiScore !== null && (
                                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${applicant.aiScore >= 70
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl relative z-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col animate-slide-up">
                        ...
                        <div className="p-6 sm:p-8">
                            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 min-h-[400px]">
                                {viewingResume.text.startsWith('data:application/pdf;base64') ? (
                                    <iframe
                                        src={viewingResume.text}
                                        className="w-full h-[600px] rounded-lg border border-gray-100"
                                        title={`Resume - ${viewingResume.name}`}
                                    />
                                ) : viewingResume.text.startsWith('data:image/') ? (
                                    <img
                                        src={viewingResume.text}
